...
class TestPlaceholders:
    def test_a_forgotten_slot_is_caught(self, rd):
        assert rd._placeholders(["<la 3e question hit du 22/08>"])

    def test_a_real_question_is_not(self, rd):
        assert rd._placeholders(["Quel processeur a mon NiPoGi ?"]) == []

    def test_an_ellipsis_is_caught(self, rd):
        assert rd._placeholders(["Quel est le ..."])
