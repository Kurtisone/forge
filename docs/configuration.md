...
| `SYSADMIN_USE_HARNAIS` | Observe the machine through the Harnais (CPU, RAM, containers, failed systemd units, logs) and hand the model a Context Builder text instead of a raw log block. Every line it states is either a `[fact]` with a source and a timestamp or an `[unobserved]` saying what could not be read. On a question naming **no** target this replaces `journalctl -k`; on one that **names** a target (route C) the validated per-unit collection becomes a Fact rendered beside the machine's state, with `journalctl -k` dropped as not being the subject. Measured with `bench/context_builder_ab.py` (Qwen3.8-9B, 2026-09-14): the log block loses two fixtures of four **outright** — a container restarted 247 s ago, and 94 % memory use — because a log cannot contain either, so resampling changes nothing; on a named unit whose own logs are routine startup notices it read one of those notices as the cause, which is run #be385d16's shape exactly. The confabulation that held route C back (*"plante **car** le socket de Podman n'existe pas"*) needs the **subject** to be unobserved, which cannot happen past `collect_node`'s three refusals. Set `false` for the old path on both branches, no code change | `true` |
