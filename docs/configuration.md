...
| `RECALL_EXPANSION` | What to do when the distance cutoff drops everything, instead of refusing: `off`, `terms` (deterministic rewrites of the question — the frame and the stopwords removed), or `llm` (those, plus one model call asking for three queries written with the words the *answer* would use). Runs on that path only, so it costs nothing on a question that already works and changes no distance `RECALL_MAX_DISTANCE` was calibrated against. **Inert with no cutoff set** — nothing is ever dropped, so there is never a failure to rescue; Forge says so at startup. Measure it with `bench/in_container.sh recall_expansion` before turning it on | `off` |
