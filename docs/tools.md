...
| `research` | `ENABLED_TOOLS=chat,code,research` | Search → fetch top results → synthesize one answer, as a single deterministic call (see below). Searches the **public web** and knows nothing about this machine: when the question names one of your running containers, the answer carries a line saying so — measured 2026-09-12, 2 of 18 real research calls were questions about a local service, and one of them is the example the router prompt gives for what must never be routed here. The answer ends with the pages it actually opened, listed **in code** from the URLs the graph fetched — never asked of the model, because a plausible URL a model produced is indistinguishable from one it read. Search results whose page was not opened are counted, not named: they contributed a snippet, and calling that a source is the overstatement a sources block exists to prevent |
| `sysadmin` | `ENABLED_TOOLS=chat,code,sysadmin` | Discover → collect → synthesize: diagnoses a service/system problem from real logs, read-only always — never restarts/stops anything. Discovery drops `.device` units: udev generates one per device node, it has no journal of its own, and on the Deck they were **half the names the user was shown** out of 522. Four nodes written in code refuse instead of diagnosing — target not found, collection failed, nothing collected, nothing observed at all — because a model handed evidence about something else answers fluently anyway. A target the user never typed is not one of them: the router copies names out of the history, so a hint appearing in neither the message nor the router's restatement is treated as an artefact and the run observes the machine instead (measured on five production turns, 2026-09-14). Works kernel-log-only out of the box; see [`deploy/README.md`](../deploy/README.md) for read-only proxies giving it real `journalctl`/`systemctl`/`podman` access |
