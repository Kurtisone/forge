...
| `research` | `ENABLED_TOOLS=chat,code,research` | Search → fetch top results → synthesize one answer, as a single deterministic call (see below). Searches the **public web** and knows nothing about this machine: when the question names one of your running containers, the answer carries a line saying so — measured 2026-09-12, 2 of 18 real research calls were questions about a local service, and one of them is the example the router prompt gives for what must never be routed here |
