...
| `sysadmin` | `ENABLED_TOOLS=chat,code,sysadmin` | Discover → collect → synthesize: diagnoses a service/system problem from real logs, read-only always — never restarts/stops anything. Discovery drops `.device` units: udev generates one per device node, it has no journal of its own, and on the Deck they were **half the names the user was shown** out of 522. Three nodes written in code refuse instead of diagnosing — target not found, collection failed, nothing collected — because a model handed evidence about something else answers fluently anyway. Works kernel-log-only out of the box; see [`deploy/README.md`](../deploy/README.md) for read-only proxies giving it real `journalctl`/`systemctl`/`podman` access |
